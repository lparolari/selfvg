...
class ReferitDatum:
    ...
    def get_locations(self):
        ...
        locations = [get_location(query) for query in self.get_queries()]
        ...
        for label in set(labels):
            ...
            if len(indexes_by_label) > 1:
                for box_index in indexes_by_label:
                    ...
                    relations[box_index][2] = (
                        1 if cx != leftmost and cx != rightmost else 0
                    )
                    relations[box_index][3] = 1 if cy == topmost else 0
                    relations[box_index][4] = 1 if cy == bottommost else 0
                    relations[box_index][5] = (
                        1 if cy != topmost and cy != bottommost else 0
                    )
